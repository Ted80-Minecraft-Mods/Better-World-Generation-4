...
public class ChunkGeneratorSkyBlock implements IChunkProvider
{
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
        ...
        if(par2 == 0 && par3 == 0)
        {
			if(isNether)
			{
				(new DecoSurvival(8)).generate(endWorld, endRNG, 10, 80, 0);
			}
			else
			{
				(new DecoSurvival(6)).generate(endWorld, endRNG, size, 70, 0);
				(new DecoSurvival(7)).generate(endWorld, endRNG, 0, 80, 60);
			}
        }
    }
