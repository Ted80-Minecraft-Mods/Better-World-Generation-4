		...
		{	
			...
			for(int mainx = x - 1; mainx < x + 2; mainx++)
			{
				for(int mainz = z - 1; mainz < z + 2; mainz++)
				{
					for(int mainy = y - 5; mainy < y + 10; mainy++)
					{
						...
						{
							par1World.setBlockWithNotify(mainx, mainy, mainz, Block.cobblestoneMossy.blockID);
						}
						else
						{
							par1World.setBlockWithNotify(mainx, mainy, mainz, Block.cobblestone.blockID);
						}
					}
				}
			}

			par1World.setBlockWithNotify(x + 2, y + 6, z, Block.cobblestoneMossy.blockID);
			par1World.setBlockWithNotify(x + 3, y + 6, z, Block.cobblestoneMossy.blockID);
			par1World.setBlockWithNotify(x + 3, y + 5, z, Block.cobblestoneMossy.blockID);
			par1World.setBlockWithNotify(x + 3, y + 4, z, Block.cobblestoneMossy.blockID);
			...
			for(int earz = z - 2; earz < z + 3; earz++)
			{
				for(int eary = y + 2; eary < y + 8; eary++)
				{
					if (par2Random.nextInt(2) != 0)
					{
						par1World.setBlockWithNotify(x, eary, earz, Block.cobblestoneMossy.blockID);
					}
					else
					{
						par1World.setBlockWithNotify(x, eary, earz, Block.cobblestone.blockID);
					}
				}
			}
			...
			for(int headz = z - 1; headz < z + 2; headz++)
			{
				for(int heady = y + 2; heady < y + 10; heady++)
				{
					if(heady > y + 7 || heady < y + 5)
					{
						if (par2Random.nextInt(2) != 0)
						{
							par1World.setBlockWithNotify(x + 2, heady, headz, Block.cobblestoneMossy.blockID);
						}
						else
						{
							par1World.setBlockWithNotify(x + 2, heady, headz, Block.cobblestone.blockID);
						}
					}	
				}
			}
