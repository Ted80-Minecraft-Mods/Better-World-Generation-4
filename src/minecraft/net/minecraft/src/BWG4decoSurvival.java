...
public class BWG4decoSurvival extends WorldGenerator
{
    ...
    public boolean generate(World par1World, Random par2Random, int x, int y, int z)
    {
		if(survivalobject == 1)//SURVIVAL ISLAND DIRT BLOCK WITH CHEST
		{
			...
			for (int x1 = 0 + x; x1 <= 2 + x; x1++) 
			{
				...
				{			
					for (int y1 = -2 + y; y1 <= 0 + y; y1++)
					{
						if(y1 == 0 + y)
						{
							par1World.setBlockWithNotify(x1, y1, z1, Block.grass.blockID);
						}
						else
						{
							par1World.setBlockWithNotify(x1, y1, z1, Block.dirt.blockID);
						}	
					}
				}
			}
			...
			for (int x3 = xx - 2; x3 <= xx + 2; x3++) //Leaves layer 1
			{
				...
				{			
					...
					{
						par1World.setBlockWithNotify(x3, y3, z3, Block.leaves.blockID);
					}
				}	
			}		
			for (int x4 = xx - 1; x4 <= xx + 1; x4++) //Leaves layer 2
			{
				...
				{			
					par1World.setBlockWithNotify(x4, yy + 4, z4, Block.leaves.blockID);
				}	
			}		
			for (int y2 = yy; y2 <= yy + 4; y2++) //LOG
			{
				par1World.setBlockWithNotify(xx, y2, zz, Block.wood.blockID);
			}
			par1World.setBlockWithNotify(xx + 1, yy + 5, zz, Block.leaves.blockID);
			par1World.setBlockWithNotify(xx - 1, yy + 5, zz, Block.leaves.blockID);
			par1World.setBlockWithNotify(xx, yy + 5, zz + 1, Block.leaves.blockID);
			par1World.setBlockWithNotify(xx, yy + 5, zz - 1, Block.leaves.blockID);
			par1World.setBlockWithNotify(xx, yy + 5, zz, Block.leaves.blockID);
			
			//CHEST
			par1World.setBlockWithNotify(xx - 1, yy, zz, Block.chest.blockID);
		...
		else if(survivalobject == 3)//NETHER WARD
		{
			for (int var6 = 0; var6 < 64; ++var6)
			{
				...
				if (par1World.isAirBlock(var7, var8, var9) && par1World.getBlockId(var7, var8 - 1, var9) == Block.netherrack.blockID)
				{
					...
					par1World.setBlockAndMetadata(var7, var8, var9, Block.netherStalk.blockID, 1);
				}
			}
		...
		else if(survivalobject == 4)//PARADISE PALM TREES
		{
			...
			{
				if (par1World.isAirBlock(x, s, z)) { par1World.setBlockAndMetadata(x, s, z, Block.wood.blockID, 3); }
			}
			if (par1World.isAirBlock(x, y + treeheight, z)) { par1World.setBlockAndMetadata(x, y + treeheight, z, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x, y + treeheight - 1, z + 1)) { par1World.setBlockAndMetadata(x, y + treeheight - 1, z + 1, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x, y + treeheight - 1, z - 1)) { par1World.setBlockAndMetadata(x, y + treeheight - 1, z - 1, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x + 1, y + treeheight - 1, z)) { par1World.setBlockAndMetadata(x + 1, y + treeheight - 1, z, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x - 1, y + treeheight - 1, z)) { par1World.setBlockAndMetadata(x - 1, y + treeheight - 1, z, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x, y + treeheight - 1, z + 2)) { par1World.setBlockAndMetadata(x, y + treeheight - 1, z + 2, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x, y + treeheight - 1, z - 2)) { par1World.setBlockAndMetadata(x, y + treeheight - 1, z - 2, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x + 2, y + treeheight - 1, z)) { par1World.setBlockAndMetadata(x + 2, y + treeheight - 1, z, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x - 2, y + treeheight - 1, z)) { par1World.setBlockAndMetadata(x - 2, y + treeheight - 1, z, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x, y + treeheight - 2, z + 3)) { par1World.setBlockAndMetadata(x, y + treeheight - 2, z + 3, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x, y + treeheight - 2, z - 3)) { par1World.setBlockAndMetadata(x, y + treeheight - 2, z - 3, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x + 3, y + treeheight - 2, z)) { par1World.setBlockAndMetadata(x + 3, y + treeheight - 2, z, Block.leaves.blockID, 3); }
			if (par1World.isAirBlock(x - 3, y + treeheight - 2, z)) { par1World.setBlockAndMetadata(x - 3, y + treeheight - 2, z, Block.leaves.blockID, 3); }
		...
		else if(survivalobject == 6)//SURVIVAL SKYBLOCK MAIN
		{
			...
			for(blx = 0 + bx; blx <= bx + 6 ; blx++)
			{
				for(bly = 0 + by; bly <= by + 2; bly++)
				{
					for(blz = 0 + bz; blz <= bz + 6 ; blz++)
					{
						if(bly == 2 + by)
						{
							par1World.setBlockWithNotify(blx, bly, blz, Block.grass.blockID);
						}
						else
						{
							par1World.setBlockWithNotify(blx, bly, blz, Block.dirt.blockID);
						}	
					}
				}
			}
			for(blx = 0 + bx + 3; blx <= bx + 6 ; blx++)
			{
				for(bly = 0 + by; bly <= by + 2; bly++)
				{
					...
					{
						par1World.setBlockWithNotify(blx, bly, blz, 0);
					}
				}
			}
			
			//ADD CHEST AND BEDROCK
			par1World.setBlockWithNotify(bx + 1, by, bz + 5, Block.bedrock.blockID);
			par1World.setBlockWithNotify(bx + 1, by + 3, bz, Block.chest.blockID);
		...
		{
			//ADD CHEST AND CACTI
			par1World.setBlockWithNotify(x + 1, y + 2, z + 1, Block.cactus.blockID);
			par1World.setBlockWithNotify(x, y + 2, z, Block.chest.blockID);
		...
		else if(survivalobject == 8)//SURVIVAL SKYBLOCK NETHER
		{
			...
			par1World.setBlockWithNotify(x - 1, y + 2, z - 1, Block.chest.blockID);
