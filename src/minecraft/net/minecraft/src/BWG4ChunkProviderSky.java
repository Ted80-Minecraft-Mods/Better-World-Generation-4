...
public class BWG4ChunkProviderSky implements IChunkProvider
{
                    ...
                    for(int l1 = 0; l1 < 4; l1++)
                    {
                        ...
                        for(int i2 = 0; i2 < 8; i2++)
                        {
                            ...
                            for(int k2 = 0; k2 < 8; k2++)
                            {
                                ...
                                if(d15 > 0.0D)
                                {
									if(nether) { l2 = Block.netherrack.blockID; }
									else { l2 = Block.stone.blockID; }
                                }
    ...
    public void populate(IChunkProvider ichunkprovider, int i, int j)
    {
		if (worldtype == 2)
		{
			...
			generatedTemperatures = worldObj.getWorldChunkManager().getColdTemperatures(generatedTemperatures, k + 8, l + 8, 16, 16);
			for(int j19 = k + 8; j19 < k + 8 + 16; j19++)
			{
				for(int j22 = l + 8; j22 < l + 8 + 16; j22++)
				{
					...
					if(d1 < 0.5D && k25 > 0 && k25 < 128 && worldObj.isAirBlock(j19, k25, j22))
					{
						...
						{
							worldObj.setBlockWithNotify(j19, 63, j22, Block.ice.blockID);					
						}
					}	
				}
		...
		else
		{
			...
			for (int lr = 0; lr < 15; ++lr)
			{
				...
				(new WorldGenHellLava(Block.lavaMoving.blockID)).generate(this.worldObj, this.rand, lr1, lr2, lr3);
			}
			...
			if (rand.nextInt(4) == 0) 
			{
				...
				(new BWG4decoSurvival(2)).generate(this.worldObj, this.rand, nt1, worldObj.getHeightValue(nt1, nt3), nt3);
			}
			...
			if (rand.nextInt(3) == 0)
			{
				...
				(new BWG4decoSurvival(3)).generate(this.worldObj, this.rand, ns1, ns2, ns3);
			}		
			...
			if (rand.nextInt(1) == 0)
			{
				...
				(new WorldGenFlowers(Block.mushroomBrown.blockID)).generate(this.worldObj, this.rand, mb1, mb2, mb3);
			}
			...
			if (rand.nextInt(1) == 0)
			{
				...
				(new WorldGenFlowers(Block.mushroomRed.blockID)).generate(this.worldObj, this.rand, mr1, mr2, mr3);
			}		
